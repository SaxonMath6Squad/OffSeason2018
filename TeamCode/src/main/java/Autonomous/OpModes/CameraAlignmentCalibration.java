...
public class CameraAlignmentCalibration extends LinearOpMode{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            if(gamepad1.a) cameraAligner.setKp(cameraAligner.getKp() + .05);
            if(gamepad1.b) cameraAligner.setKp(cameraAligner.getKp() - .05);
            if(gamepad1.x) cameraAligner.setKi(cameraAligner.getKi() + .02);
            if(gamepad1.y) cameraAligner.setKi(cameraAligner.getKi() - .02);
            if(gamepad1.right_bumper) cameraAligner.setKiMax(cameraAligner.getKiMax() + .05);
            if(gamepad1.right_trigger > .1) cameraAligner.setKiMax(cameraAligner.getKiMax()  - .05);
            cameraAligner.centerOnCryptoBoxClosestToCenter(0, columnLocations, WEST, EAST);
            // keep updating your image and column locations until you are centered
            bmp = vuforia.getImage(DESIRED_WIDTH, DESIRED_HEIGHT);
            columnLocations = cryptoFinder.findColumns(bmp, false);
            telemetry.addData("Kp","" + cameraAligner.getKp());
            telemetry.addData("Ki","" + cameraAligner.getKi());
            telemetry.addData("Ki max","" + cameraAligner.getKiMax());
            telemetry.update();
        }
